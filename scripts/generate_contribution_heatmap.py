...
def save_gif(frames: list[Image.Image], path: Path, duration: int = 110):
    ...
    sheet = Image.new(
        "RGB",
        (max(s.width for s in samples) * len(samples), max(s.height for s in samples)),
        (6, 10, 18),
    )
    offset = 0
    for sample in samples:
        sheet.paste(sample, (offset, 0))
        offset += sample.width
...
def fetch_contributions() -> tuple[date, date, dict[date, tuple[int, int]]]:
    ...
    # GitHub-style calendar columns start on Sunday.
...
def main():
    ...
    for frame_no in range(24):
        ...
        draw.text(
            (38, 23),
            "LIVE CONTRIBUTION HEATMAP / LAST 365 DAYS",
            font=font(10),
            fill=(100, 116, 139),
        )
        draw.text(
            (38, 43),
            "real public GitHub contribution history · animated scan",
            font=font(9),
            fill=(148, 163, 184),
        )
        draw.text(
        ...
        for _, _, level, col, row in visible:
            x = left + col * (cell + gap)
            y = top + row * (cell + gap)
        ...
        for fill in levels:
            draw.rounded_rectangle(
                (lx, legend_y - 2, lx + cell, legend_y + 9),
                radius=2,
                fill=fill,
            )
            lx += cell + gap + 4
